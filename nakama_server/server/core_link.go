...
func LinkFacebook(ctx context.Context, logger *zap.Logger, db *runtime.DBManager, socialClient *social.Client, router MessageRouter, userID uuid.UUID, username, appId, token string, sync bool) error {
	if token == "" {
	...
	res, err := db.Hugh_db.ExecContext(ctx, `
UPDATE users AS u
...
AND (NOT EXISTS
	...
	if facebookProfile.Email != "" {
		_, err = db.Hugh_db.ExecContext(ctx, "UPDATE users SET email = $1 WHERE id = $2", facebookProfile.Email, userID)
		if err != nil {
			var pgErr *pgconn.PgError
			if errors.As(err, &pgErr) && pgErr.Code == dbErrorUniqueViolation && strings.Contains(pgErr.Message, "users_email_key") {
...
func LinkSteam(ctx context.Context, logger *zap.Logger, db *runtime.DBManager, config Config, socialClient *social.Client, router MessageRouter, userID uuid.UUID, username, token string, sync bool) error {
