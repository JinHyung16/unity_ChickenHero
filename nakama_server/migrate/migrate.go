...
func Parse(args []string, tmpLogger *zap.Logger) {
	...
	rawURL := ms.dbAddress // "root@localhost:3306" 들어옴
	...
	*/
	...
	db.QueryRow(`SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = ?`, dbname).Scan(&schema_name)
	fmt.Println("schema_name : ", schema_name)
	if len(schema_name) == 0 {
		nakamaDBExists = false
		fmt.Println("nakamaDBExists : false")
	} else {
		nakamaDBExists = true
		fmt.Println("nakamaDBExists : true")
	}
	...
	if !nakamaDBExists {
		...
		fmt.Println("?", fmt.Sprintf("CREATE DATABASE %s", dbname))
	...
	fmt.Println(rawURL + dbname + "?parseTime=true")
	/*
	...
	*/
	db, err = sql.Open("mysql", rawURL+dbname+"?parseTime=true")
	if err != nil {
		db.Close()
		logger.Fatal("Failed to open database", zap.Error(err))
	}
...
func (ms *migrationService) parseSubcommand(args []string, logger *zap.Logger) {
	...
	flags.StringVar(&ms.dbAddress, "database.address", "root@localhost:3306", "mysql 로그인양식 username:password@address:port/dbname)")
