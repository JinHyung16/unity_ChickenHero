...
func main() {
	...
	startupLogger.Info("Nakama(Login) starting")
	...
	*/
	...
	consoleSessionCache := server.NewLocalSessionCache(config)
	...
	runtime, runtimeInfo, err := server.NewRuntime(ctx, logger, startupLogger, db, jsonpbMarshaler, jsonpbUnmarshaler, config, socialClient, sessionRegistry, sessionCache, statusRegistry, matchRegistry, tracker, metrics, streamManager, router)
	if err != nil {
	...
	apiServer := server.StartApiServer(logger, startupLogger, db, jsonpbMarshaler, jsonpbUnmarshaler, config, socialClient, sessionRegistry, sessionCache, statusRegistry, matchRegistry, matchmaker, tracker, router, streamManager, metrics, pipeline, runtime)
	consoleServer := server.StartConsoleServer(logger, startupLogger, db, config, tracker, router, streamManager, metrics, sessionCache, consoleSessionCache, loginAttemptCache, statusRegistry, statusHandler, runtimeInfo, matchRegistry, configWarnings, semver, apiServer, cookie)
...
func DBConnect(multiLogger *zap.Logger, config server.Config) (*sql.DB, string) {
	...
	db, err := sql.Open("mysql", parsedURL.String()+"nakama?parseTime=true")
