...
func NewRuntimeProviderGo(ctx context.Context, logger, startupLogger *zap.Logger, db *sql.DB, protojsonMarshaler *protojson.MarshalOptions, config Config, socialClient *social.Client, /*leaderboardCache LeaderboardCache, leaderboardRankCache LeaderboardRankCache, leaderboardScheduler LeaderboardScheduler,*/ sessionRegistry SessionRegistry, sessionCache SessionCache, statusRegistry *StatusRegistry, matchRegistry MatchRegistry, tracker Tracker, metrics Metrics, streamManager StreamManager, router MessageRouter, rootPath string, paths []string, eventQueue *RuntimeEventQueue, matchProvider *MatchProvider) ([]string, map[string]RuntimeRpcFunction, map[string]RuntimeBeforeRtFunction, map[string]RuntimeAfterRtFunction, *RuntimeBeforeReqFunctions, *RuntimeAfterReqFunctions, /*RuntimeMatchmakerMatchedFunction, RuntimeTournamentEndFunction, RuntimeTournamentResetFunction, RuntimeLeaderboardResetFunction,*/ *RuntimeEventFunctions, func() []string, error) {
	...
	//플러그인 통해서 말고 직접등록
	initializer.RegisterRpc("rpc_get_channel_list", rpcGetChannelList)
	initializer.RegisterRpc("rpc_check_account_exist", rpcCheckAccountExist)

	initializer.RegisterRpc("rpc_change_state", LocalUserState)
	initializer.RegisterRpc("rpc_list_user", ListUserStatus)
...
//플러그인 운영하기 싫어서 여기다 등록
type ResGameChannel struct {
	Id   int    `json:"id"`
...
//게임채널리스트 검색
func rpcGetChannelList(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, payload string) (string, error) {

	channel_query := `SELECT * FROM game_channel_address`

	rows, err := db.QueryContext(ctx, channel_query)
	if err != nil {
		return "", err
	}

	channels := make([]*ResGameChannel, 0)
	for rows.Next() {
		var id int
		var host string
		var port int
		...
		channels = append(channels, &ResGameChannel{Id: id, Host: host, Port: port})
	}

	channelList, _ := json.Marshal(channels)

	return string(channelList), nil
}
...
//ID 중복체크
func rpcCheckAccountExist(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, payload string) (string, error) {
	accountId := payload

	query := "SELECT id, username, email, create_time FROM users WHERE email = ?"
	...
	err := db.QueryRowContext(ctx, query, accountId).Scan(&id, &username, &email, &create_time)

	switch {
	case err == sql.ErrNoRows:
		resCheckAccount := ResCheckAccount{MessageCode: NotFound, Message: "None Exist Account"}
		jsonData, _ := json.Marshal(resCheckAccount)
		return string(jsonData), nil
	case err != nil:
		resCheckAccount := ResCheckAccount{MessageCode: ServerError, Message: err.Error()}
		jsonData, _ := json.Marshal(resCheckAccount)
		return string(jsonData), nil
	default:
		resCheckAccount := ResCheckAccount{MessageCode: Success, Message: "Already Exist Account", UserId: id, Email: email, Create_time: create_time}
		jsonData, _ := json.Marshal(resCheckAccount)
		return string(jsonData), nil
	}
}
