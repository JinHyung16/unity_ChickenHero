...
func (s *ApiServer) AuthenticateDevice(ctx context.Context, in *api.AuthenticateDeviceRequest) (*api.Session, error) {
	...
	var attemptLogin = CheckUserState(ctx, s.db, dbUserID)

	if attemptLogin == false {
		return nil, status.Error(codes.PermissionDenied, "제제된 사용자입니다.")
	}
	
	key, sessionCache := GetSessionCache(dbUserID)
	fmt.Println("get session : ", key)

	if sessionCache != nil {
		return nil, status.Error(codes.AlreadyExists, "이미 접속한 세션이 존재합니다 => "+key)
	}
	...
	//redis에 session 등록
	fmt.Println("[api_authenticate.go] set session ", dbUserID, " ", username)
	SetSessionCache(dbUserID, username, token, refreshToken, exp, refreshExp, 0)
...
func (s *ApiServer) AuthenticateEmail(ctx context.Context, in *api.AuthenticateEmailRequest) (*api.Session, error) {
	...
	if created == true {
		...
		//count
		var count int
		count_query := "SELECT COUNT(*) FROM user_db_weight"
		err := s.db.QueryRowContext(ctx, count_query).Scan(&count)
		if err != nil {
			fmt.Println(err.Error())
		}
		fmt.Println("user_db_weight count : ", count)
		rand.Seed(time.Now().UnixNano())
		randId := rand.Intn(count)
		fmt.Println("randId : ", randId)
		
		//users_database(userid, database_id)에 삽입..
		query := "INSERT INTO users_database (id, database_id) VALUES (?, (select target_db_id from user_db_weight where hash_id = ?))"
		_, err = s.db.ExecContext(ctx, query, dbUserID, targetdbId)
		if err != nil {
			fmt.Println(err.Error())
		}
		...
		query := "SELECT database_id FROM users_database WHERE id = ?"
		err := s.db.QueryRowContext(ctx, query, dbUserID).Scan(&targetdbId)
		if err != nil {
			fmt.Println(err.Error())
		}
	}
	*/
	...
	//redis에 session 등록
	fmt.Println("set session ", dbUserID, " ", username)
	SetSessionCache(dbUserID, username, token, refreshToken, exp, refreshExp, 0)
